	...
	?>
	      <div class="card-header">
                ...
                <div class="card-body collapse show">
					...
					<form class="form" method="post" enctype="multipart/form-data">
							...
							?>
								...
								<div class="form-group">
									<label for="cname">Selected Shop</label>
									<select id="select_shop" name="srider1" class="form-control" required>
									<option value="">Select a Shop</option>

									<?php 
                  $sellerid=$odata['sellerid'];
                  $full_id=$odata['pid'];
                  $explodee = explode("$;",$full_id);
                  $cat_slice_id = $explodee[0];
                  //$cat_slice_id=substr($full_id, 0,3); //Product ID
                  $neww = $con->query("select * from product where id = $cat_slice_id");
                  $newww = $neww -> fetch_assoc();
                  $shopp_id = $newww['cid'];

									$rid = $con->query("select * from category where id = $shopp_id");
                  $shop_id;
									while($ro = $rid->fetch_assoc())
									{
									?>

									<option value="<?php echo $ro['id']; ?>" <?php {echo 'selected'; } ?>><?php echo $ro['catname']; ?>
                  </option>
									<?php 
                  }
                  ?>
                  ...
                  <select id="select_rider" name="srider" class="form-control" required>
                  <option value="">Select a Delivery Boy</option>

                  <?php 
                  $sellerid=$odata['sellerid'];
                  $rid = $con->query("select * from rider where a_status=1 and status=1 and shop_id='$shopp_id'");
                ...
                <div class="card-body collapse show">
                    ...
                    <td><?php
								...
								<?php if($row['a_status'] == 0) {
								 ...
								 <a href="?oid=<?php echo $row['id'];?>"><button class="btn shadow-z-2 btn-info">Assign</button></a>
								<?php } else if($row['a_status'] == 1) { 
								?>
								
								<a href="?oid=<?php echo $row['id'];?>"><button class="btn shadow-z-2 btn-info">Reassign</button></a>
...
<script>
function nodriveralert() {
  var x = document.getElementById("nodriveralert");
...
<script>
   $(document).on('load','#select_shop',function()
  {
    var value = $(this).val(); //Shop x
    window.alert(value);
    $shop_id = value;
